...
func (c *ContainerdK8SManager) Close() error {
    slog.Info("Closing containerd connection...")
...
func (c *ContainerdK8SManager) PollContainers() []*cntr.Stats {
    ...
    if err != nil {
        slog.Error("Failed listing containerd containers", "error", err)
        return nil
    }
    ...
    slog.Debug("Polled containers", "total", len(containers), "matched", len(statsList), "skipped", skippedNonK8s)
...
func (c *ContainerdK8SManager) getContainerInfo(ctx context.Context, containerID string) ([]byte, bool, error) {
    slog.Debug("Fetching container info", "container_id", containerID[:12])
    
    // Check if container ID exists in the tracked map
    if _, exists := c.prevCpu[containerID]; !exists {
        slog.Debug("Container ID not found in tracked map, returning immediately", "container_id", containerID[:12])
    ...
    if err != nil {
        slog.Debug("Failed to load container", "container_id", containerID[:12], "error", err)
        return nil, true, fmt.Errorf("failed to load container %s: %w", containerID[:12], err)
    }

    info, err := container.Info(k8sCtx)
    if err != nil {
        slog.Debug("Failed to get container info", "container_id", containerID[:12], "error", err)
    ...
    if err != nil {
        slog.Debug("Failed to marshal container info", "container_id", containerID[:12], "error", err)
        return nil, true, fmt.Errorf("failed to marshal container info: %w", err)
    }

    slog.Debug("Successfully generated container info JSON", "container_id", containerID[:12])
...
func (c *ContainerdK8SManager) getLogs(ctx context.Context, containerID string) (string, bool, error) {
    slog.Debug("Fetching logs for container ID", "container_id", containerID[:12])
    ...
    if err != nil {
        slog.Debug("Failed to get container labels", "container_id", containerID[:12], "error", err)
    ...
    if podName == "" || containerName == "" {
        slog.Debug("Missing labels for container", "container_id", containerID[:12], "pod_name", podName, "container_name", containerName)
    ...
    slog.Debug("Searching for log files matching primary pattern", "pattern", pattern)
    matches, err := filepath.Glob(pattern)
    if err != nil || len(matches) == 0 {
        slog.Debug("Primary pattern match failed or empty, trying fallback pattern", "error", err)
        ...
        slog.Debug("Searching for log files matching fallback pattern", "pattern", fallbackPattern)
        matches, err = filepath.Glob(fallbackPattern)
        if err != nil || len(matches) == 0 {
            slog.Debug("Fallback pattern returned no matches", "pod_name", podName, "container_name", containerName)
            return "", true, fmt.Errorf("log file not found for pod %s, container %s", podName, containerName)
        }
    }

    slog.Debug("Found matching log files", "count", len(matches), "matches", matches)
    ...
    for _, m := range matches {
        if info, err := os.Stat(m); err == nil {
            ...
            slog.Debug("Failed to stat log file candidate", "path", m, "error", err)
        }
    }

    slog.Debug("Selected newest log file", "path", logFile, "mod_time", newestTime)

    // Tail the log file efficiently
    logs, err := tailFile(logFile, dockerLogsTail)
    if err != nil {
        slog.Debug("Failed to tail log file", "path", logFile, "error", err)
    ...
    slog.Debug("Successfully read logs", "chars", len(logs), "path", logFile)
...
func readNetworkStats(pid uint32) (uint64, uint64) {
    ...
    file, err := os.Open(path)
    if err != nil {
        slog.Debug("Failed to open network stats file", "path", path, "error", err)
        return 0, 0
    }
    defer file.Close()
    ...
    if !scanner.Scan() {
        slog.Debug("Failed to read header line 1", "path", path)
        return 0, 0
    }
    if !scanner.Scan() {
        slog.Debug("Failed to read header line 2", "path", path)
        return 0, 0
    }
    ...
    for scanner.Scan() {
        ...
        if errRx == nil && errTx == nil {
            ...
            slog.Debug("Failed to parse network fields", "path", path, "iface", ifName, "rx_error", errRx, "tx_error", errTx)
        }
    }

    if err := scanner.Err(); err != nil {
        slog.Debug("Error scanning network stats", "path", path, "error", err)
    }
